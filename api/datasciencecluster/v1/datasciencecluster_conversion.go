...
func (c *DataScienceCluster) ConvertTo(dstRaw conversion.Hub) error {
	...
	// If a round-trip stash annotation is present, restore the full AIGateway spec
	// from it, overriding the MaaS-only migration above. This preserves sub-component
	// state (e.g. BatchGateway) that v1 cannot represent natively. Falls through to
	// the MaaS migration result if the annotation is absent or corrupt.
